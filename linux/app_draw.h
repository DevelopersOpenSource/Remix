...
static void DrawCoverCircle(Img* img,const RectF& r,Color pen,float penW,float rotation){
    ...
    gfx::FillEllipse(r,Argb(255,18,20,30));
    if(img&&img->ok) gfx::DrawImgCircle(img,r,rotation);
    float hr=r.Width*.11f; gfx::FillEllipse(cx-hr,cy-hr,hr*2,hr*2,Argb(255,7,9,18));
...
static void DrawShapeToggle(Color ab,Color white,Color gray){
    ...
    Color sbg=Argb(255,13,16,30), spn=Argb(255,60,64,88);
    DrawRoundRect(st,S(10),&sbg,&spn,1.2f);
    bool cd=g_cfg.artShape==L"cd";
    RectF hl(cd?st.X+st.Width/2.f:st.X,st.Y,st.Width/2.f,st.Height);
    Color hbg=ToGdi(g_theme.accent,50);
    DrawRoundRect(hl,S(9),&hbg,nullptr);
    gfx::TextRect(L"QUAD",RectF(st.X,st.Y,st.Width/2.f,st.Height),S(14),cd?gray:white,true,gfx::Center,true);
    gfx::TextRect(L"CD",RectF(st.X+st.Width/2.f,st.Y,st.Width/2.f,st.Height),S(14),!cd?gray:ab,true,gfx::Center,true);
}
...
static void DrawStatusToast(int w,int h){
    ...
    Color bg=Argb(230,12,15,30), pn=ToGdi(g_theme.accent);
    DrawRoundRect(b,S(10),&bg,&pn,1.2f);
...
static void DrawVolumeBar(Color ab,Color gray,float barH){
    ...
    RectF vt((float)R_vol.left,(float)R_vol.top,(float)(R_vol.right-R_vol.left),barH); Color vb=Argb(255,40,42,60); DrawRoundRect(vt,2,&vb,nullptr);
...
static void DrawLibBar(Color ab,Color white,Color gray){
    ...
    if(R_searchBox.right>R_searchBox.left){
        ...
        Color fb=Argb(235,10,13,26), ln=g_searchFocus?ToGdi(g_theme.accent):Argb(255,50,54,76);
        DrawRoundRect(b,S(9),&fb,&ln,g_searchFocus?1.5f:1.f);
...
static void DrawPlaylistCards(Color ab,Color white,Color gray){
    ...
    for(int k=0;k<n&&k<(int)R_plCards.size();k++){
        ...
        Color cb=Argb(255,9,12,25); Color cp=hot?ToGdi(g_theme.accent,220):Argb(255,40,44,65);
        ...
        Color plate=Argb(255,18,21,34); DrawRoundRect(art,10,&plate,nullptr);
        ...
        gfx::TextRect(isAll?std::to_wstring(count)+(count==1?L" faixa na biblioteca":L" faixas na biblioteca"):PlaylistCardSubtitle(pi),RectF(tx,card.Y+S(66),tw,S(18)),S(11),ab,false,gfx::Near,false,gfx::EllipsisChar);
...
static void DrawPickMark(size_t i,const RECT& rr){
    ...
    gfx::FillEllipse(c,sel?ToGdi(g_theme.accent):(have?Argb(255,60,64,88):Argb(215,12,15,30)));
...
static void DrawNormal(int w,int h){
    ...
    DrawAppBackground(w,h,Argb(255,5,7,18));
    Color accent=ToGdi(g_theme.accent); Color ab=accent, white=C_WHITE, gray=C_GRAY, dimB=Argb(255,44,48,70);
    ...
    if(R_listBtn.right>R_listBtn.left){
        ...
        Color lpn=lm?ToGdi(g_theme.accent):Argb(255,90,94,118);
    ...
    Color navB=ToGdi(cNav), playP=ToGdi(cPlay), playB=ToGdi(cPlay);
    // ---------------- painel grande do player ----------------
    {
        RectF pnl=RF(R_playerPanel);
        Color pb=Argb(255,8,11,24); DrawRoundRect(pnl,18,&pb,nullptr);
        {   // LED: contorno neon do painel (brilho, velocidade e efeito das configuracoes)
            ...
            for(int i=0;i<3;i++){ Color lc=ToGdi(cLed,(BYTE)(la/(i+1))); RectF gr(pnl.X-(float)i,pnl.Y-(float)i,pnl.Width+2.f*i,pnl.Height+2.f*i); DrawRoundRect(gr,18.f+i,nullptr,&lc,1.f+i*.5f); }
        }
        BYTE ra=RunnerAlpha();
        if(g_cfg.runnerOn&&ra>6) DrawRunnerRect(pnl,18.f,ResolveCustom(g_cfg.runnerColor),ra,g_runnerPhase);
        ...
        if(isCd){
            DrawCoverCircle(g_coverImg,cvr,accent,2,g_player.playing?g_rotation:0);
            ...
            Color plate=Argb(255,16,19,32); DrawRoundRect(cvr,14,&plate,&accent,2);
        ...
        std::wstring artist=ct?ct->artist:L"—";
        {
            ...
            DrawMarqueeText(artist,fArtistFit,false,ar,ab,false,S(26),NowMs());
        ...
        gfx::Line((float)wl,syl,(float)wr,syl,3,Argb(75,85,87,110));
        gfx::Line((float)wl,syl,(float)wl+(wr-wl)*frac,syl,3,accent);
        float kx=(float)wl+(float)(wr-wl)*frac;
        gfx::FillEllipse(kx-S(6),syl-S(6),S(12),S(12),ab);
        gfx::StrokeEllipse(kx-S(6),syl-S(6),S(12),S(12),1.4f,Argb(220,255,255,255));
        ...
        gfx::StrokeEllipse(RF(R_play),2.4f,playP);
        ...
        Color grip=Argb(160,150,153,175);
    ...
    if(g_cfg.listMode!=0){
        ...
        for(size_t i=0;i<g_tracks.size() && i<R_cardRects.size();++i){
            ...
            RectF row=RF(rr);
            BYTE ra=RunnerAlpha();
            Color rb=cur?ToGdi(g_theme.accent,40):Argb(210,9,12,25);
            Color rp=cur?ToGdi(g_theme.accent,190):Argb(255,32,36,56);
            DrawRoundRect(row,S(10),&rb,&rp,1.2f);
            if(cur&&g_cfg.runnerOn&&ra>6){
            ...
            else {Color plate=Argb(255,18,21,34);DrawRoundRect(art,6,&plate,nullptr);Img* im=GetThumb(g_tracks[i].coverPath);if(im)gfx::DrawImg(im,art);}
            ...
            {   // linha: artista + estado do canal de streaming (tocando / na fila: pronta, carregando...)
                ...
                gfx::TextRect(g_tracks[i].artist,RectF(tx,(float)rr.top+S(31),aw,S(16)),fSmall,ab,false,gfx::Near,false,gfx::EllipsisWord);
            }
    ...
    for(size_t i=0;i<g_tracks.size() && i<R_cardRects.size();++i){
        ...
        DrawRoundRect(card,16,&cb,&cp,1.5f);
        BYTE ra=RunnerAlpha();
        if(cur&&g_cfg.runnerOn&&ra>6) DrawRunnerRect(card,16.f,ResolveCustom(g_cfg.runnerColor),(BYTE)(ra*.55f),g_runnerPhase+.5f);
...
static void DrawVertical(int w,int h){
    DrawAppBackground(w,h,Argb(255,4,6,14)); Color accent=ToGdi(g_theme.accent); Color ab=accent, white=C_WHITE, gray=C_GRAY;
    ...
    if(g_cfg.artShape==L"square"){
        Color plate=Argb(255,16,19,32); DrawRoundRect(art,14,&plate,&accent,2);
    ...
    } else DrawCoverCircle(g_coverImg,art,accent,2,g_player.playing?g_rotation:0);
    if(g_cfg.particlesOn&&FxOn()){
    ...
    DrawCameraIcon(R_verticalCoverButton,ToGdi(g_theme.accent,220));
    ...
    // Caixas adaptativas entre a arte e os botoes: escalas grandes nao se sobrepõem.
    {
        ...
        DrawMarqueeText(artist,faFit,false,arV,ab,true,S(26),NowMs());
    ...
    COLORREF cNav=ResolveCustom(g_cfg.btnNavColor),cPlay=ResolveCustom(g_cfg.btnPlayColor);Color navB=ToGdi(cNav),playP=ToGdi(cPlay),playB=ToGdi(cPlay);
    float tSec=NowMs()/1000.0f;
    ...
    Color dimBar=Argb(255,50,52,70);
    ...
    float sy=bottom+8;
    gfx::Line((float)sl,sy,(float)srx,sy,3,Argb(75,85,87,110));gfx::Line((float)sl,sy,(float)(sl+sw*frac),sy,3,accent);
    float kx=sl+sw*frac;gfx::FillEllipse(kx-6,sy-6,12.f,12.f,ab);gfx::StrokeEllipse(kx-6,sy-6,12.f,12.f,1,Argb(220,255,255,255));
    ...
    gfx::StrokeEllipse(RF(R_play),2.2f,playP);
    ...
    DrawChromeButtons(white,gray);
    BYTE a=LedAlpha(g_glowPhase,35,180);COLORREF cLed=ResolveCustom(g_cfg.ledColor);
